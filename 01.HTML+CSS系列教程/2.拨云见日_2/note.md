...
img标签默认是一个inline类型的，因此它的对齐方式是跟文字有关系的，在设置高度时，图片跟容器底部会有一些空隙，造成这样的原因是因为图片的对齐方式是按照文字基线对齐的，而不是按照文字底线对齐的，如图：

![image-20201228163647291](note_image/image-20201228163647291.png)
...
文字的默认值是 vertical-align: baseline;基线对齐
```

 写具体页面的时候或一个布局效果的时候的步骤：

首先写html结构，然后对css重置样式，然后写具体样式。

代码文件位置：[src/22_CSS默认样式.html](./src/22_CSS默认样式.html)

CSS重置代码文件位置：[other/css-reset重置代码.md](./other/css-reset重置代码.md)
